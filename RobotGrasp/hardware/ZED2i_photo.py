import cv2
from ZED2i_camera import ZED2iCamera as Camera
import time

# 初始化相机
camera = Camera(device_id='34680631',
                image_resolution='HD720',
                image_fps=5
                )
camera.connect(enable_fill_mode=True,
                # depth_mode='ULTRA',
                # depth_mode='QUALITY',
                # depth_mode='NEURAL',
                depth_mode='NEURAL+',
...
while True:
    ...
    depth_image = image['aligned_depth']
    camera.depth = depth_image / 1000
    ...
    if key & 0xFF == ord('s'):
        ...
        camera.save_photo(rgb_name=rgb_name, depth_name=depth_name)
    if key & 0xFF == ord('c'):
